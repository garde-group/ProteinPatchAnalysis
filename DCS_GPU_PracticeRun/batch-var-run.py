...
Version of keras script adjusted for varying batch size parameter
batch-var-run.py contains final run file
...
class TimeHistory(keras.callbacks.Callback):
    ...
    #    print('Evaluating: batch {} begins at {}'.format(batch, datetime.datetime.now().time()))
        ...
        print('TEST TIME', self.start)

    def on_epoch_end(self, batch, logs=None):
    #    print('Evaluating: batch {} ends at {}'.format(batch, datetime.datetime.now().time()))
        self.end=time.time()
        #print('END TIME',self.start,self.end,self.end-self.start)
...
# FIT MODEL: --------------------------------------------------------------
history = vae.fit(x=x_train[0:10000], y=None,shuffle=True,epochs=2,batch_size=batch_size,validation_data=(x_test[0:10000], None),callbacks=[time_callback])
...
pickle_out = open("Histories/losses-256.pickle","wb")
